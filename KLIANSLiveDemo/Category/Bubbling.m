...
#define BUBBLINGX JB_ScreenWidth-30
#define BUBBLINGY JB_ScreenHeight-50
...
- (void)bubbingImage:(UIImage *)image View:(UIView *)view
{
    ...
    [self exportImage:image withColor:color View:view];
}
- (void)bubbingImages:(NSArray *)images View:(UIView *)view
{
    ...
    [self exportImage:image withColor:nil View:view];
}



-(void)exportImage:(UIImage *)image withColor:(UIColor *)color View:(UIView *)view{
    if (!image)return;
    CGFloat width = 28.0f;
    CGFloat height = width * image.size.height / image.size.width;
    CGRect frame =
    CGRectMake(BUBBLINGX - width/2 - 15,
               BUBBLINGY-height/2 - 38,
               width,
               height);
    ...
    [view.layer insertSublayer:heartLayer
                                   above:nil];
